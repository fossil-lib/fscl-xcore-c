...
XTEST_CASE(reader_let_reader_valid_input_valid) {
    ...
    TEST_ASSERT_TRUE(tscl_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
}

XTEST_CASE(reader_let_reader_valid_input_invalid) {
    // Test invalid input validation
    TEST_ASSERT_FALSE(tscl_console_in_valid_input("Enter a valid number (1-100)", xmock_validate_input));
}

XTEST_CASE(reader_let_reader_confirm_yes_no_yes) {
    // Test confirming "yes"
    TEST_ASSERT_TRUE(tscl_console_in_confirm_yes_no("Do you like programming?"));
}

XTEST_CASE(reader_let_reader_confirm_yes_no_no) {
    // Test confirming "no"
    TEST_ASSERT_FALSE(tscl_console_in_confirm_yes_no("Do you like programming?"));
}
...
XTEST_CASE(reader_let_reader_confirm_multi_menu) {
    ...
    tscl_console_in_confirm_multi_menu("Choose multiple options", menu, selections, num_options);
...
XTEST_CASE(reader_let_reader_confirm_exit) {
    ...
    TEST_ASSERT_TRUE(tscl_console_in_confirm_exit());
}
...
XTEST_CASE(reader_let_reader_confirm_exit_invalid_response) {
    ...
    TEST_ASSERT_FALSE(tscl_console_in_confirm_exit());
}
