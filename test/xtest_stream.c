...
XTEST_CASE(stream_let_open_and_close_file) {
    ...
    TEST_ASSERT_EQUAL_INT(0, stream_open(&io.stream, "testfile.txt", "w"));
...
XTEST_CASE(stream_let_write_and_read_file) {
    ...
    TEST_ASSERT_EQUAL_INT(0, stream_open(&io.stream, filename, "w"));
    size_t bytes_written = stream_write(&io.stream, content, strlen(content), 1);
    stream_close(&io.stream);
    TEST_ASSERT_EQUAL_INT(strlen(content), bytes_written);

    // Read data from the file
    char buffer[1024];
    TEST_ASSERT_EQUAL_INT(0, stream_open(&io.stream, filename, "r"));
    size_t bytes_read = stream_read(&io.stream, buffer, sizeof(buffer), 1);
    stream_close(&io.stream);
...
XTEST_CASE(stream_let_append_file) {
    ...
    TEST_ASSERT_EQUAL_INT(0, stream_open(&io.stream, filename, "w"));
    size_t bytes_written1 = stream_write(&io.stream, content1, strlen(content1), 1);
    stream_close(&io.stream);
    TEST_ASSERT_EQUAL_INT(strlen(content1), bytes_written1);

    TEST_ASSERT_EQUAL_INT(0, stream_open(&io.stream, filename, "a"));
    size_t bytes_written2 = stream_write(&io.stream, content2, strlen(content2), 1);
    stream_close(&io.stream);
...
XTEST_CASE(stream_let_save_and_reopen_file) {
    ...
    TEST_ASSERT_EQUAL_INT(0, stream_save(&io.stream, new_filename));

    // Read data from the new file and verify content
    char buffer[1024];
    TEST_ASSERT_EQUAL_INT(0, stream_open(&io.stream, new_filename, "r"));
    size_t bytes_read = stream_read(&io.stream, buffer, sizeof(buffer), 1);
    stream_close(&io.stream);
    TEST_ASSERT_EQUAL_INT(strlen(content), bytes_read);
    TEST_ASSERT_EQUAL_STRING(content, buffer);
